...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def set_ui(self):
        ...
        self.button_mode_1 = QtWidgets.QPushButton(u'Pose 추정 OFF')
        self.button_mode_2 = QtWidgets.QPushButton(u'다중인물 추정 OFF')
        self.button_mode_3 = QtWidgets.QPushButton(u'행동 추정 OFF')
        ...
        self.setWindowTitle(u'실시간 다중인물 Pose 및 행동 추정 시스템')
    ...
    def button_event(self):
        ...
        if sender == self.button_mode_1 and self.timer_camera.isActive():
            if self.__flag_mode != 1:
                self.__flag_mode = 1
                self.button_mode_1.setText(u'Pose 추정 ON')
                self.button_mode_2.setText(u'다중인물 추정 OFF')
                self.button_mode_3.setText(u'행동 추정 OFF')
            else:
                self.__flag_mode = 0
                self.button_mode_1.setText(u'Pose 추정 OFF')
                self.infoBox.setText(u'카메라가 켜져 있습니다.')
        elif sender == self.button_mode_2 and self.timer_camera.isActive():
            if self.__flag_mode != 2:
                self.__flag_mode = 2
                self.button_mode_1.setText(u'Pose 추정 OFF')
                self.button_mode_2.setText(u'다중인물 추정 ON')
                self.button_mode_3.setText(u'행동 추정 OFF')
            else:
                self.__flag_mode = 0
                self.button_mode_2.setText(u'다중인물 추정 OFF')
            ...
            if self.__flag_mode != 3:
                ...
                self.button_mode_1.setText(u'Pose 추정 OFF')
                self.button_mode_2.setText(u'다중인물 추정 OFF')
                self.button_mode_3.setText(u'행동 추정 ON')
            else:
                self.__flag_mode = 0
                self.button_mode_3.setText(u'행동 추정 OFF')
                self.infoBox.setText(u'카메라가 켜져 있습니다.')
            ...
            if self.timer_camera.isActive() == False:
                ...
                self.infoBox.setText(u'카메라가 꺼져 있습니다.')
    ...
    def show_camera(self):
        ...
        if ret:
            if self.__flag_mode == 1:
                ...
                self.infoBox.setText(u'현재 사람의 행동을 추정합니다.')
                humans = poseEstimator.inference(show)
                ori = np.copy(show)
                show, joints, bboxes, xcenter, sk= TfPoseEstimator.get_skeleton(show, humans, imgcopy=False)
                height = show.shape[0]
    ...
    def closeEvent(self, event):
        ...
        msg = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Warning, u"종료", u"종료하시겠습까?")

        msg.addButton(ok, QtWidgets.QMessageBox.ActionRole)
        msg.addButton(cancel, QtWidgets.QMessageBox.RejectRole)
        ok.setText(u'예')
        cancel.setText(u'아니오')
