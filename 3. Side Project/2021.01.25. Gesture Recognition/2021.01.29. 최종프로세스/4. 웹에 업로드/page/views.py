...
def index(request):
    ...
    # # CNN 모델 load
    # tf_model = load_model("CNN_best_model_98_tf1.h5")
    ...
    #     image, detected_humans = TfPoseEstimator.draw_humans(
    #         image, humans, imgcopy=False)

    #     for coors in detected_humans:
    #         # 가로 200pixel 세로 300pixel 이하 제외
    #         #         if w < 100 and h < 200: continue
    #         cv2.rectangle(image, (coors[0], coors[1]),
    #                       (coors[2], coors[3]), (0, 0, 255), 3)

    #         # image resizing and reshape (Preprocessing)
    #         input_image = image[int(coors[1]):int(
    #             coors[3]), int(coors[0]):int(coors[2])]
    ...
    #         # model prediction
    #         result = [np.argmax(value) for value in tf_model.predict(X)]
