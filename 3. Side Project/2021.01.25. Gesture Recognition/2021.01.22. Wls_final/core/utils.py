...
def draw_bbox2(image, bboxes, label = None , count = 9999 ,classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    num_classes = len(classes)
    image_h, image_w, _ = image.shape
    hsv_tuples = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]
    colors = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuples))
    ...
    for i in range(num_boxes[0]):
        ...
        # c1, c2 = (coor[1], coor[0]), (coor[3], coor[2])
        # cv2.rectangle(image, c1, c2, bbox_color, bbox_thick)
        # print('name: %s' %(classes[class_ind])+'  '+label, count)
        # print(c1)
        # print(c2)
        x = coor[1]
        y = coor[0]
        ...
        if abs(x-x2) < 200 and abs(y-y2) < 200:
            continue

        img_trim = image[int(y):int(y2), int(x):int(x2)]

        if label == "pedestrian":
            folder_name = "1. Pedestrian"
        elif label == "lier":
            folder_name = "2. Lier"
        elif label == "sitter":
            folder_name = "3. Sitter"
        elif label == "taxier":
            folder_name = "4. Taxier"
        else:
            folder_name = "Else"
        cv2.imwrite('./3. YOLOed Datasets/' + folder_name + '/' + label +'_%d.jpg' % count, img_trim)
        print("Saved")
        # if show_label:
        #     bbox_mess = '%s: %.2f' % (classes[class_ind], score)
        #     t_size = cv2.getTextSize(bbox_mess, 0, fontScale, thickness=bbox_thick // 2)[0]
